# 안녕하세요, 프론트엔드 개발자 배상준입니다.

웹 · 데스크탑(Electron) · WebView(Flutter) 등 다양한 환경에서  
안정적으로 동작하는 프론트엔드 아키텍처를 설계하고 구축합니다.

Next.js 기반 웹서비스, 인증 구조(httpOnly 쿠키),  
스크래핑 자동화(Electron + Puppeteer), UI/UX 리팩토링(dnd-kit) 등을 경험하며  
복잡한 문제를 단순한 구조로 만드는 것을 중요하게 생각합니다.

---

## Tech Stack

### Frontend
![TS](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white)
![JS](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?logo=nextdotjs&logoColor=white)

### State & Data
![Zustand](https://img.shields.io/badge/Zustand-4E5D94?logo=react&logoColor=white)
![Recoil](https://img.shields.io/badge/Recoil-3578E5?logo=recoil&logoColor=white)
![React Query](https://img.shields.io/badge/React_Query-FF4154?logo=reactquery&logoColor=white)
![SWR](https://img.shields.io/badge/SWR-000000?logo=vercel&logoColor=white)

### UI / Style
![Tailwind](https://img.shields.io/badge/TailwindCSS-06B6D4?logo=tailwindcss&logoColor=white)
![shadcn/ui](https://img.shields.io/badge/shadcn%2Fui-000000?logo=shadcnui&logoColor=white)
![Sass](https://img.shields.io/badge/Sass-CC6699?logo=sass&logoColor=white)

### Architecture / Platform
![Electron](https://img.shields.io/badge/Electron-47848F?logo=electron&logoColor=white)
![Flutter WebView](https://img.shields.io/badge/Flutter_WebView-02569B?logo=flutter&logoColor=white)
![Puppeteer](https://img.shields.io/badge/Puppeteer-40B5A4?logo=puppeteer&logoColor=white)
![GraphQL](https://img.shields.io/badge/GraphQL-E10098?logo=graphql&logoColor=white)
![Turborepo](https://img.shields.io/badge/Turborepo-000000?logo=vercel&logoColor=white)

### Infra / Tools
![Vercel](https://img.shields.io/badge/Vercel-000000?logo=vercel&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?logo=amazonaws&logoColor=white)
![pnpm](https://img.shields.io/badge/pnpm-F69220?logo=pnpm&logoColor=white)

---

## Portfolio
(추후 Notion/웹 포트폴리오 링크 예정)

---

## Contact
**Email:** tkdwns1324@gmail.com
